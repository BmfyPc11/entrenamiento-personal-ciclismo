...
/*
  Radio para reconocer que una cima ya tiene nombre guardado.

  Antes era el mismo que usa agruparAscensiones (250 m) a proposito,
  para que un nombre puesto una vez valiera para todas las repeticiones
  del mismo puerto. Se bajo a 50 m porque con 250 m un nombre se
  "filtraba" a subidas cercanas pero distintas (varias subidas cortas
  a menos de 250 m entre si, aunque no fueran la misma). El precio: en
  una subida donde el GPS varie mas de 50 m entre repeticiones, alguna
  puede quedarse sin el nombre aunque agruparAscensiones si la una al
  mismo puerto.
...
/* La persistencia (antes localStorage, ahora la API de Postgres) vive
   en ./nombresCache.js, no aqui: este modulo se queda sin tocar la red
   ni el navegador, tal y como dice la cabecera del archivo. */
