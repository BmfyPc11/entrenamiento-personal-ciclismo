...
/**
 * Catálogo de segmentos marcados a mano (ver Perfil, modo "marcado"), guardados
 * por coordenadas para poder reconocerlos en cualquier salida que pase por el
 * mismo sitio. Igual que nombres_cima: no hay update/delete de una fila suelta,
 * el cliente ya calcula el array final correcto y aquí solo se reemplaza entero.
 */
export async function listarSegmentosManuales() {
  const { rows } = await pool.query(
    'SELECT id, nombre, lat_inicio, lon_inicio, lat_fin, lon_fin, metros FROM segmentos_manuales'
  );
  return rows.map((r) => ({
    id: r.id,
    nombre: r.nombre,
...
export async function guardarSegmentosManuales(definiciones) {
  ...
  try {
    await client.query('BEGIN');
    await client.query('DELETE FROM segmentos_manuales');

    for (const d of definiciones) {
      await client.query(
        `INSERT INTO segmentos_manuales (id, nombre, lat_inicio, lon_inicio, lat_fin, lon_fin, metros)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [d.id, d.nombre, d.latInicio, d.lonInicio, d.latFin, d.lonFin, d.metros ?? null]
      );
    }
