...
/*
  Comprobacion ligera para el aviso de "salidas sin sincronizar" al abrir
  la app: solo los ids de las ultimas "cuantas" actividades, una sola
  pagina y sin el enriquecimiento de athlete_count que hace
  traerActividades (aqui no hace falta ese dato solo para contar cuantas
  son nuevas). Una peticion a Strava, no una por actividad.
*/
export async function ultimasActividades(cuantas) {
  const r = await api('/athlete/activities', { per_page: cuantas, page: 1 });
  if (r.error) return { error: r.error, limite: r.limite };
