...
export async function traerActividades(maxPaginas = 5) {
  ...
  /* athlete_count no viene en el listado de /athlete/activities: hay que
     pedir cada actividad individual. Se hace en paralelo para no ralentizar
     demasiado, pero con un delay entre bloques para no saturar la API. */
  const actualizarPersonas = async () => {
    const bloqueSize = 5;
    for (let i = 0; i < resultado.length; i += bloqueSize) {
      await Promise.all(
        resultado.slice(i, i + bloqueSize).map(async (s) => {
          const { datos } = await api(`/activities/${s.id}`);
          if (datos?.athlete_count) s.personas = datos.athlete_count;
        })
      );
      if (i + bloqueSize < resultado.length) await new Promise((r) => setTimeout(r, 100));
    }
  };
