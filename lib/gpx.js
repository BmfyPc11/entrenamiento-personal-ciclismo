import {
  detectarPuertos, repartoDureza, tramoDureza, num, km, metrosPorKm,
  encontrarTodosLosSegmentosManualesEnSalida,
} from './metrics.js';
...
/* ============================================================
   Nombres de las subidas de una ruta.
   ============================================================ */

/*
  Nombre de cada puerto detectado, tomado del catalogo de segmentos
  marcados a mano -los mismos de "Tus ascensos"- cruzando por
  coordenadas. Un puerto que no coincida con ningun segmento del catalogo
  se queda con "Subida N", igual que el perfil de una salida real deja en
  "Segmento N" lo que no esta marcado.

  Antes esto se resolvia consultando OpenStreetMap (Overpass) cima a cima
  y cacheando el resultado en la tabla nombres_cima. Se quito: la unica
  fuente de nombres ahora es lo que el ciclista ya ha marcado y nombrado
  el mismo, sin depender de un servicio externo ni de una cache aparte.

  El GPX no trae latlng en streams (una ruta planificada no se ha
  rodado), pero datos.puntos va en paralelo a streams.distancia -misma
  longitud, mismo indice-, asi que el latlng que necesita
  encontrarTodosLosSegmentosManualesEnSalida se arma aqui a partir de el.

  Un puerto detectado y una ocurrencia del catalogo se dan por la misma
  subida si sus tramos se solapan al menos a la mitad del mas corto: pie
  y cima casi nunca caen en el mismo indice exacto (cada uno se detecta
  con su propio criterio), pero si describen el mismo trozo de carretera
  se solapan de sobra.
*/
export function nombrarPuertosRuta(datos, puertos, definicionesSegmentos) {
  if (!puertos?.length) return [];
  const generico = (i) => `Subida ${i + 1}`;
  if (!definicionesSegmentos?.length || !datos?.puntos?.length) {
    return puertos.map((_, i) => generico(i));
  }

  const streams = {
    distancia: datos.streams.distancia,
    latlng: datos.puntos.map((p) => [p.lat, p.lon]),
  };
  const ocurrencias = encontrarTodosLosSegmentosManualesEnSalida(streams, definicionesSegmentos);

  return puertos.map((p, i) => {
    let mejor = null, solapeMax = 0;
    for (const o of ocurrencias) {
      const solape = Math.min(o.fin, p.fin) - Math.max(o.inicio, p.inicio);
      if (solape > solapeMax) { solapeMax = solape; mejor = o; }
    }
    const largoMin = mejor ? Math.min(mejor.fin - mejor.inicio, p.fin - p.inicio) : 0;
    return mejor && solapeMax >= largoMin * 0.5 && mejor.def.nombre
      ? mejor.def.nombre
      : generico(i);
  });
}
