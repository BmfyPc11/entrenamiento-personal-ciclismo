...
*/
export function zonasGpsDudoso(cache, salidas, opciones = {}) {
  ...
  (salidas || []).forEach((s) => {
    ...
    for (let k = 2; k < d.length; k++) {
      ...
      if (Math.abs(v - vPrevia) / dt > acelMaxima) anomalias.push({ punto: ll[k], id: s.id });
    }
  });
...
/*
  ...
  Recorre las salidas y no la cache a proposito. Recorriendo la cache
  entraban TODAS las salidas descargadas, con lo que esta era la unica
  tarjeta de "Tus estadisticas" que ignoraba el filtro de fechas que
  tiene justo encima: cambiabas el rango y las otras cinco se movian y
  esta no. Recibiendo las salidas ya filtradas, el rango y las
  exclusiones vienen aplicados de serie.

  Las zonas dudosas se calculan una vez para todas las salidas y no una
  por salida: precisamente lo que las define es que la misma anomalia
  aparezca en dias distintos, asi que mirando una sola salida no se ven.
*/
export function velocidadMaximaLlano(cache, salidas) {
  const zonasDudosas = zonasGpsDudoso(cache, salidas);
  let max = null;
  (salidas || []).forEach((s) => {
    const st = cache?.[s.id];
    if (!st) return;
...
/*
  Tipo de terreno de una salida completa, con el mismo corte que ya usaba
  suelto el listado de "Salidas del intervalo": por debajo de 5 m/km es
  llano, por encima de 12 es puerto, en medio mixto. Se centraliza aqui
  para no repetir el mismo par de umbrales cada vez que algo necesita
  clasificar una salida entera (a diferencia de TRAMOS_DUREZA, que
  clasifica tramos sueltos dentro de una salida).
*/
export function tipoRuta(s) {
  if (esLlana(s)) return 'llano';
  return metrosPorKm(s) > 12 ? 'puerto' : 'mixto';
}
...
const ORDEN_TIPO = { llano: 0, mixto: 1, puerto: 2 };

/* Los ultimos N dias hasta hoy, con la salida de cada dia si la hubo. */
export function ultimosDias(salidas, cfg, zonas, umbral, n = 30) {
  const hoy = new Date();
...
export const CATEGORIAS_PUERTO = [
  { codigo: '4a', nombre: '4ª cat', hasta: 75, color: '#5BA85B' },
  { codigo: '3a', nombre: '3ª cat', hasta: 200, color: '#4A90D9' },
  { codigo: '2a', nombre: '2ª cat', hasta: 500, color: '#E0C020' },
  { codigo: '1a', nombre: '1ª cat', hasta: 750, color: '#E07B2E' },
