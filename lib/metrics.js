...
/* Paleta "Cartografico": piedra, salvia, ocre, terracota y ladrillo, en
   vez del azulado/verde/ambar/naranja/rojo original. Mismo orden y
   misma logica de lectura (frio y calmado -> caliente y exigente). */
export const COLORES_ZONA = ['#8B8567', '#7C8F5B', '#C99A3E', '#C1622D', '#A1332B'];
...
/*
  Calcula las cifras (metros, desnivel, pendiente, velocidad, VAM, FC...)
  de un tramo entre dos indices del stream completo, sin aplicar ningun
  minimo: detectarPuertos la usa para cada candidato que encuentra (y
  descarta despues los que no lleguen a sus minimos), y tambien sirve
  para construir el objeto de un puerto marcado a mano, que no pasa por
  ningun umbral -si el usuario lo ha marcado, se muestra tal cual.

  aSuavizada es opcional: detectarPuertos ya tiene el perfil suavizado
  calculado (lo necesita para dar con los tramos) y lo pasa para no
  repetir el trabajo; sin el, se recalcula aqui mismo.
*/
export function construirPuerto(streams, s, e, aSuavizada = null) {
  const { distancia: d, altitud: a0, tiempo: t, fc } = streams || {};
  if (!d || !a0 || e <= s) return null;
  const a = aSuavizada || suavizar(a0, ventanaPara(d));
...
export function detectarPuertos(streams, opciones = {}) {
  ...
  */
  const construir = (s, e) => {
    const p = construirPuerto(streams, s, e, a);
    if (!p || p.metros < minMetros || p.desnivel < minDesnivel) return null;
    return p;
  };
...
export const COLORES_TIPO = { llano: '#7C8F5B', mixto: '#C1622D', puerto: '#A1332B' };
...
export const TIPO_INSIGNIA = {
  llano: { codigo: 'LLA', fondo: 'var(--green)', tinta: '#1B1712' },
  mixto: { codigo: 'COL', fondo: 'var(--amber)', tinta: '#1B1712' },
...
export const TRAMOS_DUREZA = [
  { n: 1, hasta: 4, nombre: 'Hasta 4 %', color: '#7C8F5B', etiqueta: 'Llano o falso llano' },
  { n: 2, hasta: 7, nombre: '4 – 7 %', color: '#5C7A8C', etiqueta: 'Subida sostenida' },
  { n: 3, hasta: 10, nombre: '7 – 10 %', color: '#C99A3E', etiqueta: 'Exigente' },
  { n: 4, hasta: 15, nombre: '10 – 15 %', color: '#C1622D', etiqueta: 'Dura' },
  { n: 5, hasta: 20, nombre: '15 – 20 %', color: '#A1332B', etiqueta: 'Muy dura' },
  { n: 6, hasta: Infinity, nombre: 'Más del 20 %', color: '#14100C', etiqueta: 'Pared' },
];
...
export const TRAMOS_VELOCIDAD = [
  { n: 1, hasta: 12, nombre: 'Hasta 12 km/h', color: '#6B6354', etiqueta: 'Puerto duro o parado' },
  { n: 2, hasta: 18, nombre: '12 – 18 km/h', color: '#5C7A8C', etiqueta: 'Subida o ritmo suave' },
  { n: 3, hasta: 24, nombre: '18 – 24 km/h', color: '#7C8F5B', etiqueta: 'Ritmo medio' },
  { n: 4, hasta: 30, nombre: '24 – 30 km/h', color: '#C99A3E', etiqueta: 'Buen ritmo' },
  { n: 5, hasta: 40, nombre: '30 – 40 km/h', color: '#C1622D', etiqueta: 'Rápido' },
  { n: 6, hasta: Infinity, nombre: 'Más de 40 km/h', color: '#A1332B', etiqueta: 'Punta o bajada' },
];
...
*/
/* Paleta "Cartografico": mismo orden y logica de siempre, recalentada a
   tierra -salvia, pizarra, ocre, terracota y ladrillo en vez del verde/
   azul/amarillo/naranja/rojo original. */
export const CATEGORIAS_PUERTO = [
  { codigo: '4a', nombre: '4ª', hasta: 75, color: '#7C8F5B' },
  { codigo: '3a', nombre: '3ª', hasta: 200, color: '#5C7A8C' },
  { codigo: '2a', nombre: '2ª', hasta: 500, color: '#C99A3E' },
  { codigo: '1a', nombre: '1ª', hasta: 750, color: '#C1622D' },
  { codigo: 'hc', nombre: 'HC', hasta: Infinity, color: '#A1332B' },
];
...
/*
  ...
  misma VERTIENTE: mismo pie y misma cima, cada uno dentro de su radio.

  Antes esto se decidia solo por la cima (250 m) mas una tolerancia de
  longitud (35 %) contra la mas larga vista hasta entonces -pensado para
  que un puerto afrontado por varios caminos contara como una sola
  ascension. En la practica eso tenia dos problemas: fundia caminos de
  longitud bien distinta en un solo ranking de "mejor tiempo" (8 minutos
  de una vertiente corta junto a 16 de otra larga, aunque las dos esten
  bien medidas), y un candidato mal cortado un dia concreto -una parada a
  media subida que detectarPuertos interpreta como el final del tramo-
  podia colarse en el grupo grande si su longitud quedaba, por poco,
  dentro del margen.

  Ahora cada vertiente es su propia ascension, y su pie/cima no salen de
  lo que detecto cada salida el dia que se subio -eso es justo lo que
  fallaba-, sino que se fijan una vez (el candidato mas largo visto, la
  mejor aproximacion a la longitud real: uno truncado por una parada
  nunca puede ser mas largo que el de verdad) y luego CADA salida se
  remide contra esas mismas coordenadas, igual que ya hace
  encontrarSegmentoManual con los segmentos marcados a mano -de hecho
  reutiliza su mismo idxCercano.
*/
export function agruparAscensiones(salidas, cache, excluidas, definicionesSegmentos, opciones = {}) {
  const { radio = 250, radioPie = 300 } = opciones;

  const ordenadas = [...(salidas || [])].sort((a, b) => (a.fecha < b.fecha ? -1 : 1));
  const conStreams = ordenadas.filter((s) => !excluidas?.has(s.id) && cache?.[s.id]);

  /* Segmentos manuales ya reconocidos en cada salida -se calcula una vez
     y se reutiliza en las dos pasadas de abajo-: un puerto detectado que
     sea el mismo tramo no cuenta tambien como ascension automatica, ya
     esta en "Mis segmentos" con el nombre que le haya puesto el usuario. */
  const rangosManualesPorSalida = new Map();
  for (const s of conStreams) {
    const st = cache[s.id];
    rangosManualesPorSalida.set(s.id,
      (definicionesSegmentos || []).flatMap((def) => encontrarTodosLosSegmentosManuales(st, def)));
  }

  /* ---------- pasada 1: descubrir candidatos (igual que antes) ---------- */
  const descubiertos = []; // { p, pie, cima } -con coordenadas
  const sinCoordenadas = []; // { s, p } -sin latlng, se agrupan aparte por forma

  for (const s of conStreams) {
    const st = cache[s.id];
    const candidatos = detectarPuertos(st, { minMetros: 600, minDesnivel: 40, minPend: 3 });
    const puertos = descartarSolapados(candidatos, rangosManualesPorSalida.get(s.id));
    for (const p of puertos) {
      const cima = st.latlng ? st.latlng[p.fin] : null;
      const pie = st.latlng ? st.latlng[p.inicio] : null;
      if (cima && pie) descubiertos.push({ p, pie, cima });
      else sinCoordenadas.push({ s, p });
    }
  }

  /* ---------- pasada 1.5: canonizar vertientes ---------- */
  const vertientes = [];
  for (const { p, pie, cima } of descubiertos) {
    /* De entre las vertientes ya vistas que encajen (pie y cima dentro de
       su radio), se elige la de PIE MAS CERCANO -no la primera que
       encaje-: con muchos candidatos acumulados (series/intervalos
       repetidos en la misma cuesta, cada uno con su propio ruido de GPS)
       quedarse con la primera que cumple depende del orden en que se
       procesan las salidas y puede fragmentar una sola vertiente real en
       varias, cada una con un puñado de intentos en vez de todos juntos. */
    let mejor = null, mejorDist = radioPie;
    for (const v of vertientes) {
      if (distanciaGeo(v.cima, cima) >= radio) continue;
      const dPie = distanciaGeo(v.pie, pie);
      if (dPie < mejorDist) { mejorDist = dPie; mejor = v; }
    }
    if (!mejor) vertientes.push({ pie, cima, metros: p.metros });
    else if (p.metros > mejor.metros) Object.assign(mejor, { pie, cima, metros: p.metros });
  }

  /* Fusion final: el agrupado de arriba es voraz -cada candidato solo se
     compara contra lo ya visto, nunca se revisa hacia atras- asi que un
     par de candidatos madrugadores con algo de ruido de GPS pueden fundar
     dos vertientes separadas que, vistas ya todas juntas, estan clarisimas
     de sobra dentro de su radio la una de la otra. Se repite hasta que una
     vuelta entera no funde nada mas, porque fusionar dos vertientes puede
     acercar la resultante a una tercera que antes no encajaba. Con pocas
     vertientes (unas pocas decenas) esto sale barato aunque sea O(n³) en
     el peor caso. */
  for (let cambiado = true; cambiado;) {
    cambiado = false;
    for (let i = 0; i < vertientes.length; i++) {
      for (let j = vertientes.length - 1; j > i; j--) {
        const a = vertientes[i], b = vertientes[j];
        if (distanciaGeo(a.pie, b.pie) < radioPie && distanciaGeo(a.cima, b.cima) < radio) {
          if (b.metros > a.metros) Object.assign(a, b);
          vertientes.splice(j, 1);
          cambiado = true;
        }
      }
    }
  ...
  /* ---------- pasada 2: remedir CADA salida contra CADA vertiente ---------- */
  const grupos = [];
  for (const v of vertientes) {
    const intentos = [];
    for (const s of conStreams) {
      ...
      /* Una salida puede pasar por esta vertiente mas de una vez -series,
         intervalos repetidos en la misma cuesta- y cada pasada cuenta como
         un intento propio, igual que si fueran salidas distintas: por eso
         se busca desde el principio y, tras cada coincidencia (encuentre
         o no una cima valida), se sigue buscando la SIGUIENTE a partir de
         ahi, en vez de quedarse con la primera o rendirse en cuanto una
         falla. El limite de vueltas es solo una salvaguarda -no deberia
         hacer falta ni de lejos con salidas normales- para no arriesgarse
         a un bucle infinito. */
      let desde = 0;
      for (let vuelta = 0; vuelta < 40; vuelta++) {
        const inicio = idxCercano(st.latlng, v.pie, radioPie, desde);
        if (inicio == null) break;

        /* La busqueda de la cima se acota a una distancia razonable a
           partir del pie (el doble de lo mas largo visto para esta
           vertiente, para admitir un dia mucho mas flojo sin cerrarse de
           mas): sin este limite, en una salida que vuelve a pasar cerca
           del mismo sitio mas tarde -una vuelta, una salida en bucle- el
           punto mas cercano a la cima real puede estar kilometros
           despues, y construirPuerto acaba fabricando una "ascension"
           de toda la salida. */
        const fin = idxCercanoAcotado(st, v.cima, radio, inicio + 1, v.metros * 2);
        if (fin == null) { desde = inicio + 1; continue; }

        const p = construirPuerto(st, inicio, fin);
        if (p && descartarSolapados([p], rangosManualesPorSalida.get(s.id)).length) {
          intentos.push({ salidaId: s.id, salidaNombre: s.nombre, fecha: s.fecha, cima: st.latlng[fin], ...p });
        }
        desde = fin + 1;
      }
    }
    if (!intentos.length) continue; // no deberia pasar -viene de un candidato real- pero por si acaso

    intentos.sort((a, b) => (a.segundos || 1e9) - (b.segundos || 1e9));
    /* La referencia del grupo es el ascenso mas largo registrado. */
    const masLargo = intentos.reduce((a, b) => (b.metros > a.metros ? b : a));

    grupos.push({
      id: `asc-${masLargo.salidaId}-${masLargo.inicio}`,
      cima: v.cima,
      metros: masLargo.metros,
      desnivel: masLargo.desnivel,
      pendiente: masLargo.pendiente,
      pendienteMax: masLargo.pendienteMax,
      streamsId: masLargo.salidaId,
      puertoRef: masLargo,
  ...
  /* ---------- sin coordenadas: agrupacion por forma, igual que antes ----------
     Sin latlng el anclaje por coordenadas no vale de nada: se cae a
     comparar longitud y pendiente aproximadas, menos fiable pero mejor
     que duplicar cada tramo sin GPS como una ascension nueva. */
  for (const { s, p } of sinCoordenadas) {
    const intento = { salidaId: s.id, salidaNombre: s.nombre, fecha: s.fecha, cima: null, ...p };
...
/* ============================================================
   Segmentos marcados a mano (ver Perfil, modo "marcado").

   A diferencia de un puerto detectado, un segmento manual no se puede
   volver a encontrar aplicando un criterio sobre el perfil: su huella
   es geografica (las coordenadas de su pie y su cima, ver
   lib/segmentosManualesCache.js), no una forma en la altimetria. Estas
   dos funciones son el equivalente, para ese caso, de detectarPuertos +
   agruparAscensiones.
   ============================================================ */

/* Bastante para absorber el ruido normal del GPS entre dos pasadas por el
   mismo sitio, poco para confundir dos sitios que de verdad son
   distintos. */
const RADIO_SEGMENTO_MANUAL = 100;

/*
  Cuanto puede alejarse el recorrido de un punto guardado (pie o cima),
  tras haber pasado ya cerca de el, sin dar la pasada por terminada: una
  carretera que hace una curva o un cambio de rasante cerca del punto
  puede entrar y salir de su radio dos veces seguidas en unos pocos
  cientos de metros -el mismo sitio, la misma pasada- antes de asentarse
  en el punto real. Bastante para absorber eso; poco para confundirlo con
  la vuelta siguiente de un circuito o con un camino de vuelta que pasa
  otra vez cerca de la misma cima, que quedan mucho mas lejos. Se usa en
  idxPrimeraPasadaCerca (pie) e idxCercanoAcotado (cima).
*/
const MARGEN_PASADA_CERCA = RADIO_SEGMENTO_MANUAL * 3;

/*
  Cuanto se solapan dos tramos del MISMO stream (mismo espacio de indices):
  interseccion sobre union. Sirve para decidir si un puerto detectado
  automaticamente es en realidad el mismo tramo que ya esta marcado a
  mano, para no contarlo dos veces (una en "Mis ascensiones" o en
  "Detalles de la ruta", y otra en "Mis segmentos" con el nombre que de
  verdad le ha puesto el usuario).
*/
function seSolapan(a, b, minimo = 0.5) {
  const interseccion = Math.min(a.fin, b.fin) - Math.max(a.inicio, b.inicio);
  if (interseccion <= 0) return false;
  const union = Math.max(a.fin, b.fin) - Math.min(a.inicio, b.inicio);
  return interseccion / union >= minimo;
}

/* Los candidatos (puertos detectados) que no sean, en realidad, alguno de
   los rangos ya reconocidos a mano (segmentos manuales de esta misma
   salida, ver encontrarSegmentoManual). */
export function descartarSolapados(candidatos, rangos) {
  if (!rangos || !rangos.length) return candidatos;
  return candidatos.filter((p) => !rangos.some((r) => seSolapan(p, r)));
}

/* El pie y la cima pueden coincidir por coordenadas y aun asi tratarse de
   otro camino -una carretera que cruza la de siempre casi por los mismos
   dos puntos, un desvio que vuelve a juntarse mas adelante- asi que la
   longitud real recorrida es la salvaguarda contra eso. Sin ella, un
   cruce cercano bastaria para colar un segmento que en realidad es mas
   largo (o mas corto) que el marcado. */
const TOL_LONGITUD_SEGMENTO = 0.25;

/* Indice del punto de "ll" (streams.latlng) mas cercano a "punto", entre
   "desde" y el final, o null si ninguno cae a menos de "radio". */
function idxCercano(ll, punto, radio, desde = 0) {
  let mejor = null, dist = radio;
  for (let i = desde; i < ll.length; i++) {
    if (!ll[i]) continue;
    const d = distanciaGeo(ll[i], punto);
...
/*
  Igual que idxCercano, pero sin mirar mas alla de "distanciaMax" metros
  recorridos desde "desde" -usado en agruparAscensiones para buscar la
  cima de una vertiente sin dejar que una salida que vuelve a pasar cerca
  del mismo sitio mas tarde (una vuelta, una salida en bucle) fabrique una
  "ascension" del tamano de media salida: el punto geograficamente mas
  cercano a la cima puede estar kilometros despues del pie, pero eso ya
  no es esta vertiente.

  Dentro de esa ventana, se queda con el mejor punto de la PRIMERA pasada
  cerca de "punto" (mismo criterio y mismo margen que idxPrimeraPasadaCerca,
  ver su comentario) en vez del mas cercano de TODA la ventana: cerca de
  una cima real es facil que un camino de vuelta, un mirador o un tramo
  llano en la cumbre pase otra vez cerca de las mismas coordenadas un
  poco mas adelante -a veces incluso mas cerca que el primer paso-, y
  quedarse con ese inflaria la longitud del tramo mas de lo que su
  comprobacion de tolerancia (ver TOL_LONGITUD_SEGMENTO) esta dispuesta a
  perdonar.
*/
function idxCercanoAcotado(streams, punto, radio, desde, distanciaMax) {
  const { latlng: ll, distancia: d } = streams;
  const limite = d[desde - 1] + distanciaMax;
  let mejor = null, dist = radio, idxUltimaDentro = null;
  for (let i = desde; i < ll.length && d[i] <= limite; i++) {
    if (!ll[i]) continue;
    const dd = distanciaGeo(ll[i], punto);
    if (dd < radio) {
      if (mejor == null || dd < dist) { dist = dd; mejor = i; }
      idxUltimaDentro = i;
    } else if (mejor != null) {
      if (d[i] - d[idxUltimaDentro] > MARGEN_PASADA_CERCA) break;
    }
  }
  return mejor;
}
...
/*
  Busca donde cae un segmento manual dentro de esta salida.

  El pie se busca en todo el recorrido (el mas cercano a las coordenadas
  guardadas); la cima se busca solo DESPUES del pie hallado, nunca antes:
  asi una salida de ida y vuelta que pasa dos veces por el mismo sitio no
  cuela un intervalo con la cima por delante del pie. El precio, asumido
  igual que en nombres.js: en una ida y vuelta solo se reconoce el paso
  en el que el pie cae mas cerca, no los dos.

  Encontrado el intervalo, se descarta si su longitud real (la del
  stream, no la linea recta entre pie y cima) se aleja de def.metros mas
  de TOL_LONGITUD_SEGMENTO: pie y cima pueden coincidir por coordenadas
  aunque el camino real entre uno y otro sea otro bien distinto. Sin
  def.metros -catalogos guardados antes de que este campo existiera- no
  se puede comprobar, asi que se deja pasar.

  Devuelve null si no hay streams, si el pie o la cima no caen dentro del
  radio en esta salida -lo normal, para casi todas: un segmento marcado
  en una ruta no aparece en el resto-, o si la longitud no cuadra.
*/
export function encontrarSegmentoManual(streams, def, radio = RADIO_SEGMENTO_MANUAL) {
  return encontrarTodosLosSegmentosManuales(streams, def, radio)[0] ?? null;
}

/*
  Como idxCercano, pero en vez de "el punto mas cercano de todo lo que
  queda por delante" se queda con el mejor punto de la PRIMERA pasada que
  entra dentro del radio -en cuanto el recorrido se aleja de verdad tras
  haber entrado (mas de MARGEN_PASADA_CERCA desde el ultimo punto dentro
  del radio), deja de mirar mas alla.

  La diferencia importa en un circuito con varias vueltas: idxCercano
  compara TODAS las pasadas que quedan por delante entre si, y si una
  posterior queda -por puro ruido del GPS de ese dia- un poco mas cerca
  de las coordenadas guardadas que la que tocaba, salta directamente a
  ella y la intermedia se queda sin comprobar nunca (ni siquiera entra en
  el resultado como "no reconocida": desaparece del recorrido de busqueda
  sin dejar rastro). Aqui, en cambio, cada pasada se juzga solo contra si
  misma: la primera que entra en el radio es la que cuenta, sea o no la
  mejor de las que vendran despues.

  El margen de tolerancia (en vez de cortar en cuanto se sale del radio
  una sola vez) hace falta porque "la primera vez que se entra en el
  radio" no siempre es el mejor punto de esa misma pasada: una carretera
  que roza el radio de lejos antes de asentarse en el punto real (el caso
  real que lo delato: una vertiente que empieza a media subida de otra
  mas larga, en una curva que pasa cerca del pie dos veces seguidas) daba
  con un punto peor -mas lejos- que el que venia justo despues, e
  inflaba la longitud del tramo lo bastante como para que la comprobacion
  de mas abajo (TOL_LONGITUD_SEGMENTO) lo descartase entero.
*/
function idxPrimeraPasadaCerca(ll, punto, radio, desde = 0, d = null) {
  let mejor = null, dist = radio, idxUltimaDentro = null;
  for (let i = desde; i < ll.length; i++) {
    if (!ll[i]) continue;
    const dg = distanciaGeo(ll[i], punto);
    if (dg < radio) {
      if (mejor == null || dg < dist) { dist = dg; mejor = i; }
      idxUltimaDentro = i;
    } else if (mejor != null) {
      /* Sin distancia real (no debería pasar con streams de verdad, que
         siempre la traen junto al latlng) se cae a un margen en indices
         crudos -prudente de mas, pero solo como red de seguridad. */
      const alejado = d ? d[i] - d[idxUltimaDentro] : i - idxUltimaDentro;
      const limite = d ? MARGEN_PASADA_CERCA : 5;
      if (alejado > limite) break;
    }
  }
  return mejor;
}
...
/*
  Igual que encontrarSegmentoManual, pero devuelve TODAS las veces que esta
  salida pasa por el segmento, no solo la primera -series, intervalos
  repetidos en el mismo tramo cuentan cada uno como su propio intento,
  igual que ya hace agruparAscensiones con las vertientes automaticas
  (mismo motivo: tras cada coincidencia se sigue buscando la siguiente a
  partir de ahi, en vez de quedarse con la primera o rendirse en cuanto
  una falla la comprobacion de longitud).
*/
export function encontrarTodosLosSegmentosManuales(streams, def, radio = RADIO_SEGMENTO_MANUAL) {
  const ll = streams?.latlng, d = streams?.distancia;
  if (!ll || !ll.length) return [];

  const resultados = [];
  let desde = 0;
  for (let vuelta = 0; vuelta < 40; vuelta++) {
    const inicio = idxPrimeraPasadaCerca(ll, [def.latInicio, def.lonInicio], radio, desde, d);
    if (inicio == null) break;

    /* Igual que en agruparAscensiones: sin acotar la busqueda de la cima,
       en una salida con varias repeticiones el punto mas cercano a la
       cima real puede estar mucho mas adelante -la siguiente repeticion,
       o mas alla- e inflar el tramo con lo que hay en medio, en vez de
       cerrarse en la repeticion que tocaba. Con def.metros conocido se
       acota a un margen generoso por encima de la comprobacion de
       tolerancia de mas abajo; sin el (catalogo antiguo) se busca sin
       acotar, como antes. */
    const fin = def.metros != null && d
      ? idxCercanoAcotado(streams, [def.latFin, def.lonFin], radio, inicio + 1, def.metros * 2)
      : idxPrimeraPasadaCerca(ll, [def.latFin, def.lonFin], radio, inicio + 1, d);
    if (fin == null) { desde = inicio + 1; continue; }

    const metrosValen = def.metros == null || !d ||
      Math.abs((d[fin] - d[inicio]) - def.metros) / Math.max(d[fin] - d[inicio], def.metros) <= TOL_LONGITUD_SEGMENTO;
    if (metrosValen) resultados.push({ inicio, fin });
    desde = fin + 1;
  }
  return resultados;
}
...
/*
  Que fraccion de "corto" (el mas breve de los dos) cae dentro de "largo".
  No exige contencion exacta de indices -pie y cima de cada definicion se
  buscan cada uno con su propio radio, asi que el mismo tramo marcado dos
  veces (una vez entero, otra solo su ultima rampa) casi nunca comparte el
  indice exacto de cima o de pie, aunque en la practica sea el mismo sitio.
  0.8 es "casi todo": dos tramos que de verdad son distintos (aunque se
  crucen un poco, un puerto que empieza donde otro casi termina) no llegan
  a ese solape.
*/
function fraccionDentro(corto, largo) {
  const interseccion = Math.min(corto.fin, largo.fin) - Math.max(corto.inicio, largo.inicio);
  if (interseccion <= 0) return 0;
  return interseccion / (corto.fin - corto.inicio);
}
const SOLAPE_ANIDADO = 0.8;

/*
  Marca cada ocurrencia con "anidado": true si casi entera (SOLAPE_ANIDADO)
  cae dentro de OTRA mas larga de la misma lista -un tramo corto marcado
  dentro de uno mas grande ya existente (la ultima rampa de un puerto, un
  tramo que resulta ser un trozo de otro) no es una ascension propia cada
  vez que se sube el grande entero, es el mismo paso contado dos veces.

  Se compara SIEMPRE, tengan el mismo nombre o no -aunque compartir nombre
  normalmente significa que son vertientes de la misma subida (ver
  recogerSegmentosManuales) y cada una interesa ver con sus propios
  intentos, dos vertientes que de verdad son caminos distintos casi nunca
  se solapan tanto en una salida real (el GPS solo puede haber ido por
  uno). Si SI se solapan tanto es porque, en la practica, son el mismo
  tramo marcado dos veces.
*/
function marcarAnidados(ocurrencias) {
  const longitud = (o) => o.fin - o.inicio;
  return ocurrencias.map((o) => ({
    ...o,
    anidado: ocurrencias.some((otra) =>
      otra !== o &&
      longitud(otra) > longitud(o) &&
      fraccionDentro(o, otra) >= SOLAPE_ANIDADO
    ),
  }));
}

/*
  Todas las ocurrencias de TODAS las definiciones dadas en esta salida, sin
  las marcadas como "anidado" (ver marcarAnidados): para la ficha del
  perfil y la tabla "Detalles de la ruta" (Entrenamientos.jsx) no interesa
  pintar dos veces el mismo tramo del recorrido.
*/
export function encontrarTodosLosSegmentosManualesEnSalida(streams, definiciones, radio = RADIO_SEGMENTO_MANUAL) {
  const ocurrencias = (definiciones || []).flatMap((def) =>
    encontrarTodosLosSegmentosManuales(streams, def, radio).map((idxs) => ({ ...idxs, def })));

  return marcarAnidados(ocurrencias).filter((o) => !o.anidado);
}

/*
  Cada paso de una salida por cada segmento marcado a mano, ya medido:
  { segmentoId, inicio, segundos, vam, desnivel, metros }. Une
  encontrarTodosLosSegmentosManuales (que da los indices de cada pasada) con
  construirPuerto (que saca las cifras de esos indices). Es lo que la
  sincronizacion guarda en la tabla puertos_hechos para la pestana "Amigos".

  A diferencia de encontrarTodosLosSegmentosManualesEnSalida, NO descarta las
  ocurrencias "anidadas": aqui cada segmento del catalogo se mide por su
  cuenta -que un tramo corto caiga dentro de uno mas largo no quita que sea
  un intento propio de ese tramo corto.
*/
export function medirSegmentosManualesEnSalida(streams, definiciones) {
  const out = [];
  for (const def of definiciones || []) {
    for (const { inicio, fin } of encontrarTodosLosSegmentosManuales(streams, def)) {
...
/*
  Nombre de una definicion YA existente en todo el catalogo (no solo las
  que aparecen en esta salida) cuyo pie o cima cae muy cerca de alguno de
  los del tramo recien dibujado -o null si ninguna encaja. Sirve para que
  un segmento nuevo que comparte un extremo con otro ya marcado (la
  misma cima vista desde otro punto de partida, la misma subida vuelta a
  marcar) se cree directamente vinculado (mismo nombre, ver
  recogerSegmentosManuales) en vez de nacer suelto y obligar a
  renombrarlo a mano para juntarlo.

  Se compara por coordenadas puras, sin pasar por ningun stream: la
  definicion existente puede no haberse reconocido nunca en ESTA salida
  (por ejemplo, si aqui solo se ha subido el trozo final, sin llegar a su
  propio pie) y aun asi el tramo nuevo comparte de verdad su cima con
  ella.
*/
export function nombreSegmentoQueComparteExtremo(nuevo, definiciones, radio = RADIO_SEGMENTO_MANUAL) {
  const cerca = (lat1, lon1, lat2, lon2) => distanciaGeo([lat1, lon1], [lat2, lon2]) < radio;
  for (const def of (definiciones || [])) {
    if (
      cerca(nuevo.latInicio, nuevo.lonInicio, def.latInicio, def.lonInicio) ||
      cerca(nuevo.latInicio, nuevo.lonInicio, def.latFin, def.lonFin) ||
      cerca(nuevo.latFin, nuevo.lonFin, def.latInicio, def.lonInicio) ||
      cerca(nuevo.latFin, nuevo.lonFin, def.latFin, def.lonFin)
    ) return def.nombre;
  }
  return null;
}
...
/*
  El equivalente de agruparAscensiones para segmentos manuales: en vez
  de detectar y luego agrupar por parecido, aqui ya se conoce de
  antemano cada segmento (definiciones, el catalogo de
  segmentosManualesCache.js) y solo hace falta ver, salida a salida, si
  el recorrido pasa por el. Mismo shape de grupo que agruparAscensiones
  (intentos, mejor, veces) para poder reutilizar su misma presentacion.

  Una subida puede tener mas de una vertiente -una version reducida, un
  desvio que llega al mismo collado por otro camino- que interesa ver
  como la misma ascension, con el mismo nombre, aunque el tramo real sea
  distinto. Aqui eso se resuelve por el nombre: dos o mas definiciones con
  el mismo nombre (recortado de espacios) se vinculan solas, sin que haga
  falta fusionarlas a mano -renombrar una para que coincida con otra ya
  las junta, y renombrarla para que deje de coincidir las separa. Una
  vertiente con un nombre que no comparte nadie es, a efectos practicos,
  un grupo de una sola vertiente.

  A proposito, para los intentos en si, NO se usa aqui
  encontrarTodosLosSegmentosManualesEnSalida (el filtro que evita que un
  tramo corto salga tambien como ficha propia en el perfil/mapa cuando cae
  dentro de otro mas largo, ver Entrenamientos.jsx): ese filtro es solo
  para no repetir la misma marca visual dos veces sobre el mismo trazado,
  pero el tiempo del tramo corto sigue siendo un dato real y valido -quien
  quiera ver "cuanto tardo en este tramo concreto, incluidas las veces que
  lo hice como parte de la subida entera" tiene que seguir viendolo aqui,
  en su propia tabla de tiempos.

  Si se usa, en cambio, para marcar cada intento como "anidado" (mismo
  criterio de solape, ver marcarAnidados): un intento anidado SI cuenta
  para el total y sale en la tabla de tiempos, pero NO para decidir el
  orden de las vertientes (Ascensiones.jsx) ni cual es la "principal" mas
  abajo -contar ahi las veces que un tramo se subio solo "de paso", dentro
  de otro mas largo, inflaria su puesto por encima de vertientes que de
  verdad se suben a proposito.
*/
export function recogerSegmentosManuales(salidas, cache, excluidas, definiciones) {
  const ordenadas = [...(salidas || [])].sort((a, b) => (a.fecha < b.fecha ? -1 : 1));

  const grupos = new Map(); // clave (nombre recortado, o id si no tiene) -> vertientes
  const grupoDeDef = new Map(); // def.id -> clave, para repartir las ocurrencias sin volver a buscar
  for (const def of (definiciones || [])) {
    const clave = def.nombre?.trim() || def.id;
    if (!grupos.has(clave)) grupos.set(clave, []);
    grupos.get(clave).push(def);
    grupoDeDef.set(def.id, clave);
  }

  const intentosPorGrupo = new Map([...grupos.keys()].map((clave) => [clave, []]));
  ...
  for (const s of ordenadas) {
    ...
    /* Una sola pasada por TODAS las definiciones -no solo las del grupo
       que se este procesando- para poder marcar el solape contra
       cualquier otra, sea de este grupo o de otro. */
    const ocurrencias = (definiciones || []).flatMap((def) =>
      encontrarTodosLosSegmentosManuales(st, def).map((idxs) => ({ ...idxs, def })));

    for (const { inicio, fin, def, anidado } of marcarAnidados(ocurrencias)) {
      const p = construirPuerto(st, inicio, fin);
      if (!p) continue;
      intentosPorGrupo.get(grupoDeDef.get(def.id)).push({
        salidaId: s.id, salidaNombre: s.nombre, fecha: s.fecha,
        vertienteId: def.id, vertienteNombre: def.nombre,
        anidado,
        ...p,
      });
    }
  ...
  return [...grupos.entries()].map(([clave, vertientes]) => {
    const intentos = intentosPorGrupo.get(clave);
    intentos.sort((a, b) => (a.segundos || 1e9) - (b.segundos || 1e9));

    /*
      "principal" es la vertiente mas usada (veces NO anidadas, ver el
      comentario de mas arriba), y en empate la mas dura (mismo
      coeficiente que decide la categoria) -mismo criterio que el orden de
      las vertientes en Ascensiones.jsx. Es la que se ve en la fila
      principal (km, %, desnivel...) en vez de "mejor", que es solo el
      intento con mejor tiempo de TODAS las vertientes juntas y puede
      pertenecer a una que apenas se sube. Con una sola vertiente las dos
      coinciden siempre -son el mismo intento.
    */
    let principal = intentos[0] || null;
    if (vertientes.length > 1) {
      let mejorPuntuacion = null;
      for (const def of vertientes) {
        const intentosDef = intentos.filter((it) => it.vertienteId === def.id);
        if (!intentosDef.length) continue;
        const mejorDef = intentosDef[0];
        const coef = categoriaPuerto(mejorDef.metros, mejorDef.pendiente).coef;
        const puntuacion = { veces: intentosDef.filter((it) => !it.anidado).length, coef };
        if (!mejorPuntuacion ||
          puntuacion.veces > mejorPuntuacion.veces ||
          (puntuacion.veces === mejorPuntuacion.veces && puntuacion.coef > mejorPuntuacion.coef)) {
    ...
    return {
      id: clave,
      nombre: vertientes[0].nombre,
      vertientes,
      intentos,
      mejor: intentos[0] || null,
      principal,
      veces: intentos.length,
    };
  });
