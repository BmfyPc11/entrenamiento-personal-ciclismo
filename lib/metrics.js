...
/* ============================================================
   Evolucion por tipo de terreno.
   ============================================================ */

export const TERRENOS = [
  { id: 'llano', nombre: 'Llano', min: 0, max: 5, color: '#4A90D9' },
  { id: 'mixto', nombre: 'Mixto', min: 5, max: 15, color: '#E0C020' },
  { id: 'montana', nombre: 'Montaña', min: 15, max: Infinity, color: '#D14B42' },
];

/*
  Agrupa las salidas por tipo de terreno y devuelve, para cada grupo, la
  serie cronologica de velocidad y pulsaciones.

  La comparacion solo tiene sentido dentro del mismo terreno: ir a 20 km/h
  en el delta y a 20 km/h subiendo Montjuic no son el mismo esfuerzo, y
  mezclarlos en una sola grafica produce una linea que no significa nada.

  El indicador que de verdad interesa es la eficiencia: metros por
  pulsacion. Si sube, avanzas mas por cada latido, que es exactamente lo
  que mejora con el entrenamiento aerobico.
*/
export function evolucionPorTerreno(salidas, excluidas) {
  const val = (salidas || [])
    .filter((s) => !excluidas?.has(s.id))
    .sort((a, b) => (a.fecha < b.fecha ? -1 : 1));

  return TERRENOS.map((t) => {
    const grupo = val.filter((s) => {
      const m = metrosPorKm(s);
      return m >= t.min && m < t.max;
    });

    const puntos = grupo.map((s) => {
      const v = kmh(s);
      const fc = s.fcMedia || null;
      return {
        id: s.id,
        fecha: s.fecha,
        nombre: s.nombre,
        ...
        fc,
        vam: vamSalida(s),
        /* Metros recorridos por latido: velocidad entre pulsaciones. */
        eficiencia: fc ? (v * 1000 / 60) / fc : null,
      };
    });

    const conFC = puntos.filter((p) => p.fc);
    let tendencia = null;
    if (conFC.length >= 2) {
      const pri = conFC[0], ult = conFC[conFC.length - 1];
      tendencia = {
        dVel: ult.velocidad - pri.velocidad,
        dFC: ult.fc - pri.fc,
        dEfic: ult.eficiencia - pri.eficiencia,
        pctEfic: ((ult.eficiencia - pri.eficiencia) / pri.eficiencia) * 100,
        desde: pri.fecha,
        hasta: ult.fecha,
      };
    }

    return { ...t, puntos, conFC: conFC.length, tendencia };
  }).filter((t) => t.puntos.length > 0);
}
