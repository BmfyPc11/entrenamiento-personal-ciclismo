...
export function velocidadMaximaLlanoTramo(streams, opciones = {}) {
  ...
  const { distancia: d, altitud: a0, tiempo: t } = streams || {};
...
/* La misma busqueda sobre todas las salidas analizadas. Solo cuentan las
   que tienen streams en cache: sin distancia y altitud punto a punto no
   hay ventana de 50 m que medir. */
export function velocidadMaximaLlano(cache, excluidas) {
  let max = null;
  Object.entries(cache || {}).forEach(([id, st]) => {
    if (excluidas && excluidas.has(Number(id))) return;
    const v = velocidadMaximaLlanoTramo(st);
