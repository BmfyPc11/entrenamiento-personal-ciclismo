...
export function recogerSegmentosManuales(salidas, cache, excluidas, definiciones) {
  ...
  return (definiciones || []).map((def) => {
    ...
    for (const s of ordenadas) {
      ...
      for (const idxs of encontrarTodosLosSegmentosManuales(st, def)) {
        const p = construirPuerto(st, idxs.inicio, idxs.fin);
        if (!p) continue;
        intentos.push({ salidaId: s.id, salidaNombre: s.nombre, fecha: s.fecha, ...p });
      }
    }
