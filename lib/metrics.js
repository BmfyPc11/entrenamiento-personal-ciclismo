...
/*
  ...
  entra dentro del radio -en cuanto el recorrido vuelve a salir del radio
  tras haber entrado, deja de mirar mas alla.
...
*/
function idxPrimeraPasadaCerca(ll, punto, radio, desde = 0) {
  let mejor = null, dist = radio;
  for (let i = desde; i < ll.length; i++) {
    if (!ll[i]) { if (mejor != null) break; continue; }
    const d = distanciaGeo(ll[i], punto);
    if (d < radio) {
      if (mejor == null || d < dist) { dist = d; mejor = i; }
    } else if (mejor != null) {
      break;
    }
  }
...
export function encontrarTodosLosSegmentosManuales(streams, def, radio = RADIO_SEGMENTO_MANUAL) {
  ...
  let desde = 0;
  for (let vuelta = 0; vuelta < 40; vuelta++) {
    const inicio = idxPrimeraPasadaCerca(ll, [def.latInicio, def.lonInicio], radio, desde);
    if (inicio == null) break;
    ...
    const fin = def.metros != null && d
      ...
      : idxPrimeraPasadaCerca(ll, [def.latFin, def.lonFin], radio, inicio + 1);
    if (fin == null) { desde = inicio + 1; continue; }
